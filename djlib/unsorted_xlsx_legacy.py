...
def write_unsorted_rows(path: Path, rows: Iterable[Dict[str, str]], bucket_choices: Sequence[str]) -> None:
    ...
    # Build column letter map for Excel formula in final_filename
    _col_letters: Dict[str, str] = {}
    for _ci, _sp in enumerate(UNSORTED_COLUMNS, start=1):
        _col_letters[_sp.name] = get_column_letter(_ci)

    def _final_filename_formula(row_num: int) -> str:
        """Build an Excel formula that live-computes final_filename from editable cells."""
        a = f'{_col_letters["artist"]}{row_num}'
        t = f'{_col_letters["title"]}{row_num}'
        v = f'{_col_letters["version_info"]}{row_num}'
        b = f'{_col_letters["bpm"]}{row_num}'
        k = f'{_col_letters["key_camelot"]}{row_num}'
        fp = f'{_col_letters["file_path"]}{row_num}'
        # Extension: check common audio extensions, default .mp3
        ext = (
            f'IF(RIGHT({fp},5)=".flac",".flac",'
            f'IF(RIGHT({fp},5)=".aiff",".aiff",'
            f'IF(RIGHT({fp},4)=".wav",".wav",'
            f'IF(RIGHT({fp},4)=".m4a",".m4a",".mp3"))))'
        )
        # Version part: if non-empty and already starts with "(", use as-is; otherwise wrap in parens
        ver = (
            f'IF({v}="","",IF(LEFT({v},1)="("," "&{v}," ("&{v}&")"))'
        )
        return (
            f'=IF({a}="","Unknown Artist",{a})'
            f'&" - "'
            f'&IF({t}="","Unknown Title",{t})'
            f'&{ver}'
            f'&" ["'
            f'&IF({k}="","??",{k})'
            f'&" "'
            f'&IF(OR({b}="",{b}=0),"??",TEXT(ROUND({b},0),"0"))'
            f'&"]"'
            f'&{ext}'
        )
    ...
    for row_idx, data in enumerate(normalized_rows, start=2):
        ...
        for col_idx, spec in enumerate(UNSORTED_COLUMNS, start=1):
            # final_filename: write an Excel formula that auto-recalculates
            if spec.name == "final_filename":
                cell = ws.cell(row=row_idx, column=col_idx,
                               value=_final_filename_formula(row_idx))
                cell.fill = row_fill
