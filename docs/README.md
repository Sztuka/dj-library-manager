...
4. **ROUND — 1) Analyze+Enrich+Predict+Export** – pełna runda (analiza audio, wzbogacanie multi-source, predykcja bucketów, eksport XLSX).
...
- **`config.yml`** (zapisywany przez wizard):
  ...
  ```
  - Alternatywnie: ustaw w zmiennych środowiskowych (`LASTFM_API_KEY`, `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, `SOUNDCLOUD_CLIENT_ID`).

## Enrichment (multi-source) i nowe kolumny CSV

- Źródła: MusicBrainz, Last.fm, Spotify, SoundCloud (opcjonalnie).
- Kolumny: `genres_musicbrainz`, `genres_lastfm`, `genres_spotify`, `genres_soundcloud`, `pop_playcount`, `pop_listeners`.
- Agregat: `genre_suggest` bazuje na ważonej fuzji źródeł (Last.fm 6.0, MB 3.0, SC 2.0, SP 1.0).
...
| `python -m djlib.cli round-1`            | Złożona runda (analyze+enrich+predict+export) | (wewn. sekwencja)                        |
...
Jeśli potrzebujesz szybkiego skrótu działań: użyj `round-1`, przejrzyj XLSX i popraw `target_subfolder`, potem `round-2`.
